...
rem --- test creating a customer profile
rem --- suspect customer profiles will be in new file firm/customer/gateway/sequence, profileID
goto skipthis
	customerProfileType! = new CustomerProfileType()
        customerProfileType!.setMerchantCustomerId("Customer: "+cust_id$)
        customerProfileType!.setDescription("Customer name for " + cust_id$) 

	apiRequest! = new CreateCustomerProfileRequest()
	...
	authResponse! = new CreateCustomerProfileResponse()
	authResponse! = controller!.getApiResponse()
skipthis:


rem create customer profile ID type	

CustomerProfilePaymentType!= new CustomerProfilePaymentType()
CustomerProfilePaymentType!.setCustomerProfileId("1511243985");rem generated from above txn
rem - CustomerProfilePaymentType!.setCustomerProfileId("1511257741");rem from a prior J2Pay response
					...
					if authResponse!.getMessages().getResultCode()=MessageTypeEnum.OK
						...
						setprogbar!.setValue(sid$,"init")
						setprogbar!.setCallbackForVariableChange(sid$,"custom_event")
rem						returnCode=scall("bbj "+$22$+"are_hosted.aon"+$22$+" - -g"+gateway_id$+" -t"+authResponse!.getToken()+" -a"+masked_amt$+" -l"+launch_page$+" -u"+gw_config!.get("gatewayURL")+" -s"+sid$)
