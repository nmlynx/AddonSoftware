[[IVE_COUNT_ENTRY.WAREHOUSE_ID.AVAL]]
rem --- Set default

	user_tpl.default_whse$ = callpoint!.getUserInput()
...
rem --- Set default
	...
	user_tpl.default_cycle$ = cycle$
[[IVE_COUNT_ENTRY.LOTSER_NO.BINP]]
rem --- Is there a lot/serial#?
	...
	if !user_tpl.this_item_lot_ser then 
		util.disableField(callpoint!, "IVE_COUNT_ENTRY.LOTSER_ITEM")
	endif
...
rem --- Set default values
	...
	if user_tpl.default_whse$ <> "" then
		callpoint!.setColumnData("IVE_COUNT_ENTRY.WAREHOUSE_ID", user_tpl.default_whse$)
	endif

	if user_tpl.default_cycle$ <> "" then
		callpoint!.setColumnData("IVE_COUNT_ENTRY.PI_CYCLECODE", user_tpl.default_cycle$)
	endif
...
rem ===========================================================================
	...
	if !this_item_lot_ser then
		rem callpoint!.setColumnEnabled("IVE_COUNT_ENTRY.LOTSER_ITEM", 0)
		rem util.disableField(callpoint!, "IVE_COUNT_ENTRY.LOTSER_ITEM")
	endif
...
rem --- Parse count string, display total
	...
	count$ = cvs( callpoint!.getColumnData("IVE_COUNT_ENTRY.COUNT_STRING"), 1)
