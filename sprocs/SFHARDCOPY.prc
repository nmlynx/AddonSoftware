...
rs! = BBJAPI().createMemoryRecordSet("FIRM_ID:C(2), WO_NO:C(7*), WO_TYPE:C(1*), WO_CATEGORY:C(1*), WO_STATUS:C(1*),
:									  CUSTOMER_ID:C(1*), SLS_ORDER_NO:C(1*), WAREHOUSE_ID:C(1*),
:                                     ITEM_ID:C(1*), OPENED_DATE:C(1*), LAST_CLOSE:C(1*),TYPE_DESC:C(1*), PRIORITY:C(1*),
:                                     UOM:C(1*), YIELD:C(1*), PROD_QTY:C(1*), COMPLETED:C(1*), LAST_ACT_DATE:C(1*), ITEM_DESC:C(1*),
:									  DRAWING_NO:C(1*), REV:C(1*)")
...
rem --- Trip Read
	...
	extract record (sfe_womast_dev, key=firm_id$+from_wo$, dom=*next)
	while 1
		sfe_key$=key(sfe_womast_dev,end=*break)
		if pos(firm_id$=sfe_key$)<>1 break
		readrecord (sfe_womast_dev,key=bmm01_key$) sfe_womast$
		if cvs(thru_wo$,2)<>""
...
rem		data!.setFieldValue("TYPE_DESC",sfe_womast.std_lot_size$)
		...
		data!.setFieldValue("ITEM_DESC",sfe_womast.description$)
